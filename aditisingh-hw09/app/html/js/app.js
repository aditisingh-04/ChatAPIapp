 ...
 ************************/
const imgHost = "https://yhy7hpcmvllckflpi4frdvrwli0waxdd.lambda-url.us-east-1.on.aws/"
...
async function checkUploadImage() {
    ...
    fileName = fileName.replaceAll(' ', '-').replaceAll('+', '-').replaceAll('%20', '-')
     ...
     *      - full_url 
     ************************/
    const generateUploadURL = await fetch(imgHost+"?filename="+fileName)
    generateUploadResponse = await generateUploadURL.json()
    var status = generateUploadResponse.status;
    var uploadUrl = generateUploadResponse.upload_url;
    var thumbnailUrl = generateUploadResponse.thumbnail_url;
    var fullUrl = generateUploadResponse.full_url;
...
function makeNewChatElement(chat, position="end") {
    ...
    if (chat.image_full_url && chat.image_thumbnail_url) {
         ...
         * 
         ************************/
        var chatImageElement = document.createElement('img')
         ...
         * 
         ************************/
        newChat.appendChild(chatImageElement)
    }
